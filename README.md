...
| **AgentWatch** | **Wazuh agent health monitor — polls the Wazuh REST API on a configurable interval, detects disconnected/never-connected agents, surfaces state changes in real time, and emits structured JSON log events for SIEM ingestion** | **Operate** | **Rust, Port —** |
